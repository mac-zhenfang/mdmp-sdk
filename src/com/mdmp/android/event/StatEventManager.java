 package com.mdmp.android.event;
...
import com.mdmp.android.db.DatabaseManager;
 ...
 public class StatEventManager
 {
   ...
   static StatEventManager parse(Cursor cur)
   {
     StatEventManager event = new StatEventManager();
   ...
   static List<StatEventManager> getEvents(DatabaseManager dbManager, String event, String value, int date, boolean send)
   {
   ...
   static int[] getEventDates(DatabaseManager dbManager) {
     int[] dates = null;
     SQLiteDatabase db = dbManager.getReadableDatabase();
     Cursor cur = null;
     try {
   ...
   static void addEvent(DatabaseManager dbManager, String event, int date, int hour, String value, int count, int exit, long duration, boolean first, long time)
   {
     StatEventManager ev = new StatEventManager();
     ev.setEvent(event);
     ev.setDate(date);
     ev.setHour(hour);
     ev.setValue(value);
   ...
   static void clearAccessPath(DatabaseManager dbManager)
   {
     SQLiteDatabase database = dbManager.getWritableDatabase();
   ...
   static void clearHistoryEvents(DatabaseManager dbManager, int date)
   {
     SQLiteDatabase database = dbManager.getWritableDatabase();
     database.delete("event", "send=? and date<?", new String[] { "1", Integer.toString(date) });
   }
 
   static void addEvent(DatabaseManager dbManager, String event, int date, int hour, String value, int count, long duration, boolean first, long time)
   {
 ...
 * Qualified Name:     com.imofan.android.basic.MFStatEvent
