...
import com.mdmp.android.StatInfo;
...
public class FeedbackService {
	...
	public static void submit(Activity activity, Feedback feedback,
		...
		new Thread(new Runnable() {
			public void run() {
				if ((this.val$activity1 == null)
						|| (this.val$feedBack1 == null)
						|| (this.val$feedBack1.getFeedback().length() > 500)
						|| (this.val$feedbackSubmitListener1 == null)) {
				...
				Map maps = null;
				backJson = FeedbackService.access$000(this.val$activity1,
						this.val$feedBack1);
				...
				FeedbackService.update(this.val$activity1,
						new FeedbackReplyListener() {
							public void onDetectedNothing() {
							}

							public void onDetectedNewReplies(
									List<Feedback> feedbacks) {
							}
						});
				this.val$feedbackSubmitListener1
						.onSubmitSucceeded(this.val$feedBack1);
			}
		}).start();
	}
		...
		if (json != null) {
			try {
				...
				for (int i = 0; i < jsonArray.length(); i++) {
					Feedback fd = new Feedback(jsonArray.getJSONObject(i)
							.getString("feedback"));
					fd.setUserType(jsonArray.getJSONObject(i).getInt("by"));
					fd.setTimestamp(jsonArray.getJSONObject(i).getString(
							"timestamp"));
					list.add(fd);
				}
	...
	public static void update(Activity activity,
			FeedbackReplyListener feedbackReplyListener) {
		Activity activity1 = activity;
		...
		new Thread(new Runnable() {
			public void run() {
				if ((this.val$activity1 == null)
						|| (this.val$feedbackReplyListener1 == null)) {
					this.val$feedbackReplyListener1.onDetectedNothing();
					return;
				}

				SharedPreferences preferences = this.val$activity1
						.getSharedPreferences("mffeedbacks", 0);
				String timestamp = preferences.getString("timestamp", "0");

				String backJson = FeedbackService.access$200(
						this.val$activity1, timestamp);
				if (("".equals(backJson)) || (backJson == null)) {
					this.val$feedbackReplyListener1.onDetectedNothing();
					return;
				}
	...
	private static void saveReply(Activity activity, String json) {
		...
		editor.putString("timestamp",
				((Feedback) list.get(list.size() - 1)).getTimestamp());
		editor.commit();
		...
		if (localReply == null)
			...
			try {
				JSONObject jsonObj = new JSONObject(preferences.getString(
						"json", null));
				JSONArray jsonArray = jsonObj.getJSONArray("feedbacks");
				for (int i = 0; i < list.size(); i++) {
					JSONObject jo = new JSONObject();
	...
	private static String getSubmitResponseWithPost(Activity activity,
			Feedback mffeedBack) {
		...
		HttpConnectionParams
				.setConnectionTimeout(param, CONNECT_TIMEOUT * 1000);
		...
		params.add(new BasicNameValuePair("app_ver", StatInfo.getString(
				"app_ver", "")));
		params.add(new BasicNameValuePair("os_ver", StatInfo.getString(
				"os_ver", "")));
	...
	private static String getFbRspWithGet(Activity activity, String ts) {
		...
		String GET_FEEDBACK_URL = FEEDBACK_URL + DataAgent.getAppKey(activity)
				+ "&dev_id=" + getUUID(activity) + "&ts=" + ts;
