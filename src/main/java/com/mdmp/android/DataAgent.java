...
//	private static final String COLLECTOR = "http://localhost:8090/mdmp/update";
	// NOTICE : USE ROTER ADD , NOT localhost, THE DEVICE AND YOUR COMPUTER ARE NOT SAME COMPUTER
	private static final String COLLECTOR = "http://192.168.1.113:8090/mdmp/update";
	...
	static void sendEventsToServer(SendData data, long time) {
		...
		try {
			...
			Log.i("MDMP","Server URL:" + COLLECTOR);
			//URL url = new URL(URLEncoder.encode(COLLECTOR, "GB2312"));
			URL url = new URL(COLLECTOR);
			Log.i("MDMP","Server HOST:" + url.getHost());
			Log.i("MDMP","Server PATH:" + url.getPath());
			Log.i("MDMP","Server PORT:" + url.getPort());
			Log.i("MDMP","Server String:" + url.toString());
	...
	public static void main(String[] args) {
		...
		try {
			...
			json.put("value", "2");
			data.setJson(json);
			sendEventsToServer(data, 123456789);
		} catch (JSONException e) {
